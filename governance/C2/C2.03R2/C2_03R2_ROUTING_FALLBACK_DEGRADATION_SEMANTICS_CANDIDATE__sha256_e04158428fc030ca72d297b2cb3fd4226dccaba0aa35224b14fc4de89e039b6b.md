# C2.03R2 — Routing / Fallback / Degradation Semantics Narrow Repair Candidate
...
The user explicitly authorized **C2.03R2 narrow repair** after the C2.03R1 Independent Routing Re-Check returned `FAIL`. This successor candidate repairs only the residual `C2ROUTE-B04` activation/precedence gap. `C2ROUTE-B01..B03` remain closed by the R1 independent re-check and are not reopened. All other C2.03 semantics remain inherited unless explicitly restated here. It does not self-close `C2ROUTE-B04` and does not authorize C2.04.
...
Repair posture: `C2ROUTE-B01..B03 = CLOSED_BY_R1_INDEPENDENT_RECHECK`; `C2ROUTE-B04 = ADDRESSED_NOT_CLOSED_PENDING_R2_INDEPENDENT_ROUTING_RECHECK`.
...
### 12.4 Deterministic transition activation and precedence

**Authorization is not activation.** Presence of a structurally valid `AuthorizedRequirementTransitionInput` means only that `R -> R'` is trusted and available if the current requirement reaches the activation condition below. It MUST NOT, by presence alone, preempt an executable route path or an accepted terminal result under current `R`.

C2 evaluates one route decision with one validation precondition followed by a fixed two-pass semantic order. If `authorized_requirement_transition` is present, its Section 12.2 structural identity, `from_id`, `to_id`, and trusted `authorization_ref` requirements are validated first as route-input validity only; this pre-validation has no transition-activation authority. An invalid present transition fails closed before a route decision is emitted.

#### Pass A — ordinary current-requirement evaluation

After route-input validation, C2 computes the exact ordinary current-`R` decision **as if `authorized_requirement_transition` were absent**. This evaluation uses the pinned candidate set, route policy, C2.01 compatibility, `RouteStateKey` consumption, exact ContinuationRule matching, external-admission block semantics, and closed defaults already defined by this contract.

For transition precedence only, that ordinary evaluation is classified into exactly one of three non-wire logical classes:
...
```text
Classification is evaluated as an ordered, exhaustive partition:

1. `CURRENT_R_TERMINAL` when the ordinary action is `TERMINATE_WITH_ATTEMPT` or `TERMINATE_CANCELLED`;
2. otherwise `CURRENT_R_PROGRESS_AVAILABLE` when the current-`R` path can still advance without changing requirement, including any of:
   - before any attempt, an eligible unconsumed `RouteStateKey` under `R` is selectable;
   - `NOMINATE_SAME_ROUTE_REATTEMPT` is the ordinary action;
   - `NOMINATE_NEXT_ROUTE` resolves to an eligible unconsumed `RouteStateKey` under `R`;
   - `YIELD_FOR_EXTERNAL_ADMISSION` retains a pending `RouteStateKey` under `R`;
3. otherwise `CURRENT_R_STRUCTURALLY_EXHAUSTED` when no eligible unconsumed `RouteStateKey` under `R` is selectable under the pinned policy;
4. otherwise `CURRENT_R_TERMINAL` only for a closed ordinary terminal/default action such as policy-selected `TERMINATE_EXHAUSTED` or `REQUIRE_EXPLICIT_REQUIREMENT_DOWNGRADE` while a selectable current-`R` route still exists. Any state outside this partition is invalid and fails closed.
```

A policy-selected `TERMINATE_EXHAUSTED` while an eligible current-`R` route still exists is therefore **not** structural exhaustion for downgrade activation; it falls into `CURRENT_R_TERMINAL` and remains authoritative. Likewise a yielded external-admission state and a selected same-route reattempt are progress states rather than structural exhaustion merely because no distinct next candidate exists.

When a pre-attempt request has zero semantically/policy eligible current-`R` candidates, ordinary evaluation classifies it as `CURRENT_R_STRUCTURALLY_EXHAUSTED`. When `NOMINATE_NEXT_ROUTE` is selected but its deterministic search finds no eligible unconsumed current-`R` key, the normalized ordinary result is structural exhaustion with the existing route-level exhaustion semantics; it is not provider `NO_DATA`.

#### Pass B — authorized transition applicability

If `authorized_requirement_transition` is absent, the Pass-A decision is final for C2.03.

If the field is present, it has already passed the route-input validity precondition above. A malformed, wrong-current-requirement, ambiguous, or otherwise invalid transition input MUST fail closed; an implementation MUST NOT silently ignore an invalid present transition and continue under `R`.

For a valid transition `T`, define:
...
where `D_R` is the exact Pass-A ordinary decision and candidate selection under `R'` reuses the same pinned registry snapshot, candidate set, route policy, C2.01 compatibility predicate, deterministic ordering, and requirement-keyed consumption rules.

The precedence relation is therefore closed:

```text
CURRENT_R_TERMINAL                > authorized R->R' transition
CURRENT_R_PROGRESS_AVAILABLE      > authorized R->R' transition
CURRENT_R_STRUCTURALLY_EXHAUSTED
  + applicable valid transition  -> NOMINATE_CANDIDATE_UNDER_DOWNGRADED_REQUIREMENT
CURRENT_R_STRUCTURALLY_EXHAUSTED
  + no applicable transition     -> retain exact ordinary exhaustion/downgrade-required result
```

Thus a valid authorized transition is **latent until structural exhaustion of current `R`**. It does not override `SUCCESS + PRESENT`, terminally accepted `NO_DATA` or `PARTIAL`, `CANCELLED`, an ordinary same-route reattempt, an ordinary next-route candidate under `R`, or an external-admission yield. If current `R` is structurally exhausted and at least one `R'` route is eligible/unconsumed, the downgrade action supersedes only that ordinary exhausted result.

If current `R` is structurally exhausted but no eligible/unconsumed `R'` route exists, no `RequirementTransitionEvent` is emitted, `R'` does not become effective, and the exact ordinary current-`R` exhaustion/downgrade-required result remains. C2 does not invent a further `R''`.

Before any provider attempt, a valid transition therefore applies only when current `R` has no selectable eligible route state and `R'` does. When current `R` already has an eligible initial route, that current-`R` route is selected first and the authorized transition remains unapplied for that decision.

The above order is semantic, not implementation timing. Thread arrival, callback order, dictionary iteration, whether transition validation code executes before continuation code, or provider timing MUST NOT change the selected path. Identical pinned `RouteDecisionInput` plus identical retained lineage MUST reproduce the same Pass-A class, transition applicability result, selected action, selected candidate, and requirement-transition lineage.

### 12.5 Route degradation kinds
...
```text
C2.OPEN-006 = R2_REPAIRED_CANDIDATE_PENDING_INDEPENDENT_ROUTING_RECHECK
C2.OPEN-007 = R2_REPAIRED_CANDIDATE_AT_C2_03_SEMANTIC_LEVEL_PENDING_INDEPENDENT_ROUTING_RECHECK_AND_C2_06_WIRE_BINDING
C2.OPEN-008..015 = OPEN_UNCHANGED
```

Only the C2.03R2 Independent Routing Re-Check may close the residual repaired C2.03 semantic contract. C2.06 must still bind the C3/C6/C7 opaque references without changing this ownership seam.
...
valid AuthorizedRequirementTransitionInput -> immediate downgrade before current-R structural exhaustion
implementation evaluation order -> requirement-transition precedence
valid but inapplicable R->R' transition -> erase executable current-R path
```
